...
import {View,Text,StyleSheet,FlatList,ScrollView,Pressable,TouchableOpacity} from 'react-native'
...
export default function PostScreen() {
  ...
  const LetterFetch = async() => {
    try {
      ...
      .collection('Letter').where('postedBy',"==",firebase.auth().currentUser.uid)
      .get()
      .then((querySnapshot)=>{
    ...
    const Stdlist=({item}) =>{
      return (
        <View style={styles.mainContainer}>
          ...
          <View>
                <Text style={{fontFamily:'monospace',fontWeight:'bold',paddingLeft:10,marginBottom:10,paddingBottom:4,borderBottomWidth:1.5,borderBottomColor:'#E5E5E5'}}>{item.name}</Text>
          </View>
